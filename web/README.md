...
- 문서가 넓으면 **엣지(나가는/들어오는)가 우측 칼럼**으로, 좁으면 본문 아래로 자동 배치된다.
- 그래프는 **초기 화면에서 패널 공간을 최대한 채우도록** 레이아웃을 펼치고 맞춘다(너무 작거나 잘리지 않게).
...
- **💬 채팅 탭(WebLLM, 선택)** — 자연어로 물으면(예: "redis와 동시성이 어떻게 엮이지?") LLM이 그래프 질의(path/neighbors/SQL)로 변환→sql.js 실행→**검색 결과만 근거로** 한국어 답. 화면(목록·그래프)도 그 질의로 갱신. WebGPU 미지원 시 텍스트 검색으로 폴백(키·서버 불필요, 모델은 첫 사용 시 브라우저 다운로드).
  - **모델 선택** — 채팅 입력줄 드롭다운으로 WebLLM 모델 선택(0.5B~7B). 기본 **Qwen2.5 3B(~2GB)**. 클수록 똑똑하지만 다운로드·메모리↑. 선택은 기억되고, 로드 실패 시 더 작은 모델/검색창 폴백 안내.
  - **in-context 학습** — 시스템 프롬프트에 실제 노드 목록·타입/엣지 어휘 + 그래프의 진짜 id로 만든 few-shot 예시를 주입해 작은 모델도 형식·id를 잘 따르게 한다.
  - **대화 메모리** — 직전 대화를 기억해 후속 질문("그건 왜?")을 해석. **`지우기`** 는 화면뿐 아니라 **이 메모리도 초기화**.
  - **0건/환각 방지** — 질의가 0건이면 질문 키워드로 재검색, 그래도 없으면 지어내지 않고 "못 찾음"으로.
