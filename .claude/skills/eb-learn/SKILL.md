...
- `namespace`는 영역. `visibility` 미지정 시 **네임스페이스 기본값이 자동 적용**된다(아래 절).

## 네임스페이스 — 영역 격리 + 기본 접근제어자 (먼저 정한다)
지식을 넣기 전에 **어느 네임스페이스에 속하는지** 정한다. 네임스페이스는 영역을 격리하고(예: `personal`/`work`/`novel-<제목>`), 그 영역 새 노드의 **기본 visibility(접근제어자)** 를 결정한다.

- 새 영역(특히 소설·창작·민감한 개인 영역)이면 **먼저 등록**한다 — 창작/개인 영역은 기본 `private` 권장:
  ```bash
  python eb.py add-namespace --name novel-foo --default-visibility private --description "소설: Foo"
  ```
- 이후 그 영역 노드는 `--namespace novel-foo` 만 주면 `--visibility` 를 생략해도 **기본값(private)이 자동 적용**된다. 특정 노드만 공개하려면 `--visibility public` 으로 개별 override.
- 격리 3종 세트: ① `--namespace <슬러그>` ② 루트 `pillar` 노드 1개 + 그 영역 노드를 `part_of`로 연결(→ `eb.py components` 에서 독립 덩어리로 분리) ③ 공통 태그 1개(search에 네임스페이스 필터가 없으므로 `python eb.py search <슬러그>` 로 회수).
- 등록 현황 확인: `python eb.py namespaces` (네임스페이스별 기본 visibility·노드 수).
