...
function runSQL(s) {
  try {
    ...
    if (!seen.length) { const f = r[0]; note("SQL 결과: " + Object.entries(f).map(([k, v]) => `${k}=${v}`).join(", ") + (r.length > 1 ? ` 외 ${r.length - 1}행` : "")); return; }
...
function factIntent(q) {                            // 개수/통계는 모델 대신 결정적으로 정확히
  const raw = (q || ""), s = raw.replace(/\s/g, "");
  if (s.length > 30) return null;
  if (!/(개수|갯수|몇개|얼마나|count)/i.test(s)) return null;
  // 타입별
  if (/(타입별|종류별|타입마다|종류마다)/.test(s) && /(노드|문서|항목)/.test(s)) {
    const rs = rows("SELECT type, COUNT(*) c FROM nodes GROUP BY type ORDER BY c DESC");
    return "타입별 노드 개수 — " + rs.map((r) => `${r.type || "(없음)"} ${r.c}개`).join(", ") + ".";
  }
  // 특정 타입
  const types = [...new Set(allNodes.map((n) => n.type).filter(Boolean))], ty = types.find((t) => s.includes(t));
  if (ty) return `${ty} 타입 노드는 ${rows("SELECT COUNT(*) c FROM nodes WHERE type=?", [ty])[0].c}개예요.`;
  // 조건부: "X 포함/들어간/관련 ... 개수" → 키워드 매칭 개수
  const m = raw.match(/([0-9A-Za-z가-힣]+?)(?:이|가|을|를|와|과|에)?\s*(?:포함|들어간|들어있|담긴|관련|언급|있는|가진)/);
  if (m && m[1]) { const kw = m[1]; return `'${kw}' 관련(포함) 문서는 ${textIds(kw).length}개예요.`; }
  // 엣지
  if (/(엣지|간선|연결|링크)/.test(s)) return `엣지(연결)는 총 ${rows("SELECT COUNT(*) c FROM edges")[0].c}개예요.`;
  // 전체: 다른 내용어가 남으면(조건부) 전체로 답하지 않고 LLM에 위임
  const left = s.replace(/(전체|총|모든|노드|문서|항목|개수|갯수|몇개|얼마나|count|은|는|이|가|을|를|의|야|에요|예요|줘|해|좀|요|개|몇|\?|！|!|\.)/gi, "");
  if (!left) return `전체 노드(문서)는 ${rows("SELECT COUNT(*) c FROM nodes")[0].c}개예요.`;
  return null;
}
function nodeContext(ids) {                         // 노드 id들 → 답변 근거 텍스트(요약+본문)
  return ids.map((id) => { const n = rows("SELECT title,type,summary,body FROM nodes WHERE id=?", [id])[0] || {}; return `## ${n.title || id} (${n.type || ""})\n요약: ${n.summary || "-"}\n내용: ${(n.body || "-").slice(0, 400)}`; }).join("\n\n");
}
function buildChatTeach() {                         // in-context 학습: 실제 그래프 어휘 + few-shot
  ...
  add("노드(문서) 몇 개야?", "SELECT COUNT(*) c FROM nodes");
  add("타입별 노드 개수", "SELECT type, COUNT(*) c FROM nodes GROUP BY type");
...
async function chatAsk(q) {
  ...
  const fact = factIntent(q);                       // "전체 문서 개수" 등 통계는 정확히(모델 불필요)
  if (fact) { bubble("bot", esc(fact)); chatHistory.push({ role: "user", content: q }, { role: "assistant", content: fact }); return; }
  ...
  try {
    ...
    let ctx = "", info = "";
    if (/^\s*select\b/i.test(query)) {               // SQL: 노드 id면 노드 내용, 집계면 결과값 그대로
      try {
        const rs = rows(query), idset = new Set(allNodes.map((n) => n.id)), ids = [];
        for (const row of rs) for (const v of Object.values(row)) if (typeof v === "string" && idset.has(v) && !ids.includes(v)) ids.push(v);
        if (ids.length) { ctx = nodeContext(ids.slice(0, 8)); info = `${ids.length}건`; }
        else if (rs.length) { ctx = "쿼리 결과(표):\n" + rs.slice(0, 20).map((r) => Object.entries(r).map(([k, v]) => `${k}=${v}`).join(", ")).join("\n"); info = `${rs.length}행`; }
      } catch (e) { ctx = ""; }
    } else {
      const ids = retrieve(query).slice(0, 8);
      if (ids.length) { ctx = nodeContext(ids); info = `${ids.length}건`; }
    }
    if (!ctx) { const kw = keywordIds(q).slice(0, 8); if (kw.length) { ctx = nodeContext(kw); info = `${kw.length}건`; } }   // 0건이면 질문 키워드로
    if (!ctx) {                                      // 진짜 결과 없으면 환각 대신 솔직히
      const msg = "그래프에서 관련 내용을 찾지 못했어요. 다르게 물어봐 주세요.";
    ...
    const sys2 = `너는 이 지식그래프 전용 비서다. 아래 '근거'(노드 내용 또는 쿼리 결과)에 있는 사실/숫자만으로 한국어 2~4문장으로 답하라.
금지: 일반 상식·사전적 정의·외부 지식(예: 약자 풀이)·근거에 없는 내용. 근거에 답이 없으면 정확히 "그래프에 그 정보가 없어요"라고만 답하라.
근거:\n${ctx}`;
    ...
    thinking.innerHTML = esc(ans) + `<br><span style="opacity:.6;font-size:12px">↳ ${esc(query)} · ${info} (내부검색)</span>`;
