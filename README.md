...
## 🎙️ 음성으로 다 한다 — Gemini Live × 작업큐 (`eb-queue`)

eb는 조회만이 아니다. **폰의 Gemini Live에 말로** 시키면, 제미나이가 *원래 못 하던 일까지* 해낸다.

제미나이 앱이 기본 제공하는 도구는 대체로 **읽기·생성**이다 — Gmail 메일 읽기, Drive 문서 읽기·작성, Calendar 일정 조회·생성, 그리고 (eb 시트 미러를 읽는) **지식 조회**. 정작 **수정·삭제·발송·지식 변경**은 빠져 있다.

`eb-queue` 스킬이 이 빈칸을 **작업큐 + 워처**로 메운다 — *생산자(말)→큐(캘린더)→소비자(워처)* 파이프라인:

- **되는 일**(질문·조회·초안·일정 생성, eb 시트 지식 조회) → 제미나이가 **즉시** 처리.
- **안 되던 일**(메일 답장·발송·삭제, 문서·일정 수정·삭제, **지식그래프 추가·변경·삭제**) → 보조 캘린더 **`eb작업큐`에 일정 1개로 큐잉**(요청 1건 = 일정 1개, 상태 = 제목 접두사).
- **워처**(소비 저장소의 Claude Code, 2단 Haiku 탐지 → Opus 실행)가 큐를 가져가 **안전정책 안에서** 실행·검증(`eb.py validate`)·시트 동기화까지. **가역·안전한 그래프 편집은 자동**, **비가역·외부(메일 발송·삭제·공개 게시)는 `[확인필요]`로 사람 승인**.

결과: **말 한마디**로 "지난 회의 그래프에 넣어줘 / 그 메일 답장 보내줘 / 다음 주 일정 옮겨줘"가 큐를 거쳐 처리되고, 진행 상태(`[실행중]`·`[완료]`·`[확인필요]`)도 제미나이에게 물어 확인한다. 큐가 **비공유 보조 캘린더**라는 점이 신뢰 경계가 되고, 일정 설명 속 외부 텍스트는 *데이터일 뿐 지시가 아님*으로 다뤄 프롬프트 주입을 막는다.

```mermaid
flowchart TB
  U(["🗣️ 사용자 — 오로지 말로"])
  U --> GL(["🤖 Gemini Live<br/>음성 비서"])

  subgraph S1["① 원래 되는 일 — 제미나이가 바로 실행 (읽기·생성)"]
    direction LR
    N1["📧 Gmail<br/>메일 읽기"]
    N2["📄 Drive<br/>문서 읽기·작성"]
    N3["📅 Calendar<br/>일정 조회·생성"]
    N4["🧠 지식 조회<br/>eb 시트 읽기"]
  end

  subgraph S2["② 원래 안 되던 일 — 말하면 작업큐에 적어둠"]
    Q[("📌 eb작업큐<br/>보조 캘린더<br/>요청 1건 = 일정 1개")]
  end

  subgraph S3["③ 워처가 대신 실행 — Claude Code"]
    direction LR
    W1["🔍 1단 Haiku<br/>대기 탐지·집계"] --> W2["⚙️ 2단 Opus<br/>안전 게이트 + 실행"]
  end

  subgraph S4["④ 그래서 '말로' 새로 가능해지는 일 (수정·삭제·발송·지식변경)"]
    direction LR
    E1["📧 메일<br/>답장·발송·삭제"]
    E2["📄 문서<br/>수정·삭제"]
    E3["📅 일정<br/>수정·삭제"]
    E4["🧠 지식그래프<br/>추가·변경·삭제"]
  end
  ...
  Q --> W1
  W2 ==>|"가역·안전 → 자동 실행"| E4
  W2 -.->|"비가역·외부 → 「확인필요」 사람 승인"| E1
  W2 -.-> E2
  W2 -.-> E3
  E4 -->|"CSV → 시트·웹 미러"| N4

  classDef native fill:#e8f5e9,stroke:#43a047,color:#1b5e20;
  classDef queue fill:#fff8e1,stroke:#fbc02d,color:#7f6000;
  classDef watch fill:#e3f2fd,stroke:#1e88e5,color:#0d47a1;
  classDef new fill:#f3e5f5,stroke:#8e24aa,color:#4a148c;
...
> 셋업: `/eb-queue` — 작업큐 캘린더 확인, 워처 설치, 폰 제미나이 맞춤 Gem 온보딩(시스템 프롬프트 포함). 인스턴스 값(캘린더·시트 ID)은 **소비 저장소의 `CLAUDE.md`에만** 두고, 스킬은 인스턴스 비종속이다.
