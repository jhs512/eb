...
```bash
printf 'no,id,title,type,namespace,visibility,summary,confidence,tags,body\n' > data/nodes.csv
printf 'no,source,type,target,weight,note\n' > data/edges.csv
```
...
> **먼저 비서 이름(base `<NAME>`)을 정한다 — 기본 `eb1`.** 지식 시트 = `<NAME>`, 작업큐 캘린더 = `<NAME>-queue`. 한 사람이 여러 비서를 둘 수 있어, `eb1` 이 이미 있으면 `eb2`… 를 제안하고, 사용자가 원하면 커스텀(예 `mye`)을 받는다. 정해진 `<NAME>` 을 `eb-sheets`·`eb-queue`·CLAUDE.md에 일관되게 쓴다.
...
- **eb-queue** — 작업큐(Google Calendar 보조 캘린더 `<NAME>-queue`)·소비자 워처(Claude Code/Codex/Antigravity, 단일 인스턴스)·폰 제미나이(Saved info에 비서 지침)를 연결한다. 음성/폰으로 지식 추가·정제 요청을 큐잉하면 워처가 안전정책 안에서 처리한다(일정 CRUD는 제미나이 직접). 지식 조회가 시트 `<NAME>`을 읽으므로 `eb-sheets`(+가능하면 `eb-pages`)가 선행이면 좋다.
