...
eb를 단일 `/eb` 헬퍼 스킬이 아니라, 결정적 `eb.py` 엔진 위에 올린 **5개의 작은 스킬 묶음**으로 제품화한다 — `eb-setup`(부트스트랩) / `eb-learn`(증류) / `eb-ask`(조회) / `eb-clean`(정제) / `eb-check`(점검). [이 영상](https://www.youtube.com/watch?v=z02Y-1OvWSM)에서 영감을 받아 수집→조회→정제 루프를 스킬로 나눈 것이다.
...
- 기존 `/eb` 스킬은 `eb-learn` + `eb-ask`로 분화된다.
