...
- 스킬: `.claude/skills/eb-check/SKILL.md` 신규. stats 요약 + 리뷰 큐를 묶어 "정제 할 일 목록"으로 제시. 발견 항목을 `eb-clean`로 넘기는 흐름을 안내.
...
- [ ] `eb-check` SKILL.md가 점검 → 리뷰 큐 → `eb-clean` 인계 흐름을 안내한다.

## Blocked by

- [01-eb-ask-read-path](./01-eb-ask-read-path.md) (조회 기반 위에서 점검; 1 이후 권장)
