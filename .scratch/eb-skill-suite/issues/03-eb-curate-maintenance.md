...
- 스킬: `.claude/skills/eb-curate/SKILL.md` 신규. `search`로 중복 탐지 → `merge`로 병합, `orphans` + `suggest`로 고아에 연결 제안, `suggest`로 기존 노드 촘촘화. 매 변경 후 `validate`.
...
- [ ] `eb-curate` SKILL.md가 병합·고아 연결·촘촘화 워크플로와 변경 후 `validate`를 안내한다.

## Blocked by

- [02-eb-capture-graph-aware-write](./02-eb-capture-graph-aware-write.md) (suggest 사용)
