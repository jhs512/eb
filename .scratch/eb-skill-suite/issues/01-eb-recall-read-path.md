# eb-recall — 읽기 경로 (search 엔진 + 조회 스킬)
...
- 스킬: `.claude/skills/eb-recall/SKILL.md` 신규 작성. 기존 `search`/`neighbors`/`path`/`components`/`degree`를 조합해 "관련 서브그래프"를 제시하는 얇은 오케스트레이터. 트리거(예: "조회", "이웃", "최단 경로", "recall")를 frontmatter에 둔다.
...
- [ ] `eb-recall` SKILL.md가 search/neighbors/path/components/degree를 활용해 조회 워크플로를 안내한다.
...
- `eb-recall` SKILL.md 작성(search→neighbors→path→components/degree로 관련 서브그래프 제시, 읽기 전용).
