...
- **eb-queue** — 작업큐(Google Calendar 보조 캘린더 `eb-calendar`)·소비자 워처·폰 제미나이(Saved info에 비서 지침)를 연결한다. 음성/폰으로 지식 추가·정제 요청을 큐잉하면 워처가 안전정책 안에서 처리한다. 지식 조회가 시트 `eb`를 읽으므로 `eb-sheets`(+가능하면 `eb-pages`)가 선행이면 좋다.
