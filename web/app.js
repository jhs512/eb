...
function factIntent(q) {                            // 개수/통계는 모델 대신 결정적으로 정확히
  ...
  const count = /(개수|갯수|몇개|얼마나|count)/i.test(s);
  if (!count) return null;
  const node = /(노드|문서|항목)/.test(s), edge = /(엣지|간선|연결|링크)/.test(s);
  const types = [...new Set(allNodes.map((n) => n.type).filter(Boolean))], ty = types.find((t) => s.includes(t));
  if (/(타입별|종류별|타입마다|종류마다)/.test(s) && node) {
    const rs = rows("SELECT type, COUNT(*) c FROM nodes GROUP BY type ORDER BY c DESC");
    return "타입별 노드 개수 — " + rs.map((r) => `${r.type || "(없음)"} ${r.c}개`).join(", ") + ".";
  }
  if (ty && count) return `${ty} 타입 노드는 ${rows("SELECT COUNT(*) c FROM nodes WHERE type=?", [ty])[0].c}개예요.`;
  if (edge) return `엣지(연결)는 총 ${rows("SELECT COUNT(*) c FROM edges")[0].c}개예요.`;
  if (node || /(전체|총|모든)/.test(s)) return `전체 노드(문서)는 ${rows("SELECT COUNT(*) c FROM nodes")[0].c}개예요.`;
  return null;
}
