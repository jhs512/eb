...
- `eb-setup` — 사용자 저장소에 엔진·씨앗 데이터를 깔아주는 부트스트랩
- `eb-learn` — 원자료를 노드+엣지로 **증류**해 CSV에 반영, 추가 전 그래프를 조회하는 **그래프-인지 캡처**
- `eb-ask` — 그래프를 탐색해 답을 얻는 **조회**
- `eb-clean` — 중복 병합·고아 연결·드리프트 교정의 **정제**
- `eb-check` — 신뢰도·고아·끊긴 엣지를 모은 점검 리포트

배포는 **스킬 우선**: `skills-lock.json`에 `source: jhs512/eb`로 잠가 설치하고, `/eb-setup`이 `jhs512/eb`의 고정 ref에서 `eb.py`·씨앗 CSV를 내려받는다. 차별점은 조회·추가의 그래프 연산을 프롬프트가 아니라 **Python 엔진이 보증**한다는 것이다. 이를 위해 엔진에 `search`/`merge`/`suggest` 세 명령을 추가한다(stdlib only 유지).
...
4. 사용자로서, 나는 빈 저장소에서 `/eb-setup`을 실행해 `eb.py`·씨앗 CSV 3종을 한 번에 깔고 싶다, 그래서 곧바로 지식을 쌓기 시작할 수 있다.
5. 사용자로서, 나는 `/eb-setup`이 고정 ref에서 엔진을 가져오게 하고 싶다, 그래서 엔진이 한 곳(저장소 루트)에서만 관리되고 중복 보관되지 않는다.
6. 사용자로서, 나는 이미 `eb.py`가 있는 저장소에서 `/eb-setup`이 덮어쓰기 전에 확인하길 원한다, 그래서 기존 데이터를 잃지 않는다.
7. 지식 수집자로서, 나는 긴 텍스트·대화를 `/eb-learn`에 던지면 노드와 엣지로 **증류**되길 원한다, 그래서 직접 CSV 행을 짜지 않아도 된다.
...
13. 조회자로서, 나는 키워드로 `/eb-ask`을 호출해 제목·요약·태그·본문에서 **유사 노드**를 찾고 싶다, 그래서 관련 지식을 빠르게 찾는다.
...
18. 정제자로서, 나는 중복 노드 둘을 `/eb-clean`로 **병합**하고 싶다, 그래서 엣지가 한쪽으로 재배선되고 중복이 사라진다.
...
22. 점검자로서, 나는 `/eb-check`로 노드/엣지 수·타입 분포·평균 차수 요약을 보고 싶다, 그래서 그래프 규모를 한눈에 안다.
23. 점검자로서, 나는 `eb-check`가 **신뢰도가 낮거나 오래된 노드**를 "리뷰 큐"로 모아주길 원한다, 그래서 약한 지식을 우선 보강한다.
24. 점검자로서, 나는 `eb-check`가 끊긴 엣지·빈 필드·고아를 한 리포트로 묶어주길 원한다, 그래서 정제 작업의 할 일 목록을 얻는다.
...
- 5개 스킬을 신규 생성: `eb-setup`, `eb-learn`, `eb-ask`, `eb-clean`, `eb-check`. 각 SKILL.md는 **얇은 오케스트레이터**로, 실제 연산은 모두 `eb.py` 호출에 위임한다.
- 기존 `.claude/skills/eb`(단일 `/eb`)는 `eb-learn` + `eb-ask`로 분화한 뒤 제거한다.
...
- `eb-setup`은 실행 시 `jhs512/eb`의 **고정 ref(태그/커밋)** 에서 `eb.py`·씨앗 CSV 3종(+선택 `sync.py`·CI)을 내려받아 사용자 저장소 루트에 기록한다. 기존 파일이 있으면 **덮어쓰기 전 확인**한다.
...
**소개 문서 (README)**
- 제품 서사 우선으로 재작성: 한 줄 가치 → 문제 → 5스킬 → 결정적 엔진 차별점 → 빠른 시작(`skills-lock` → `/eb-setup` → `/eb-learn`/`/eb-ask`) → 데이터 규약·엔진 명령 전체·시트 동기화(레퍼런스).
...
- 엔진이 `eb-setup` 리소스로 중복 보관되지 않도록 fetch-from-ref를 택했으므로([ADR-0002](../../docs/adr/0002-distribution-skills-first-fetch.md)), ref 핀 갱신 절차를 README 빠른 시작에 함께 적는다.
- 다음 단계: `/to-issues`로 이 PRD를 (a) 엔진 3명령 + 테스트, (b) 5스킬 작성, (c) 배포/`eb-setup`, (d) README 재작성 단위의 이슈로 분해 → `/tdd`로 엔진부터 구현.
